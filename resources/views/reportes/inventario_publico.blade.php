...
<body class="bg-gray-100">
    ...
    <script>
        let timeoutIdPublico;
        let selectedIndexPublico = -1;

        function buscarDinamicoPublico(e) {
            const input = e.target;
            const query = input.value.trim();
            const suggestionBox = document.getElementById('suggestionBoxPublico');

            // Navegación con teclado
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp' || e.key === 'Enter' || e.key === 'Escape') {
                navegarSugerenciasPublico(e);
                return;
            }

            clearTimeout(timeoutIdPublico);

            if (query.length < 1) {
                suggestionBox.classList.add('hidden');
                suggestionBox.innerHTML = '';
                selectedIndexPublico = -1;
                return;
            }

            timeoutIdPublico = setTimeout(() => {
                realizarBusquedaAPIPublico(query);
            }, 300);
        }

        function realizarBusquedaAPIPublico(query) {
            const suggestionBox = document.getElementById('suggestionBoxPublico');
            
            suggestionBox.innerHTML = '<div class="px-4 py-3 text-sm text-gray-500">Buscando...</div>';
            suggestionBox.classList.remove('hidden');

            fetch(`/api/v1/productos/buscar?q=${encodeURIComponent(query)}&limit=10`)
                .then(response => response.json())
                .then(data => {
                    if (data.data && data.data.length > 0) {
                        renderizarSugerenciasPublico(data.data, query);
                    } else {
                        suggestionBox.innerHTML = '<div class="px-4 py-3 text-sm text-gray-500">No se encontraron productos</div>';
                    }
                })
                .catch(error => {
                    console.error('Error en búsqueda:', error);
                    suggestionBox.innerHTML = '<div class="px-4 py-3 text-sm text-red-500">Error al buscar</div>';
                });
        }

        function renderizarSugerenciasPublico(productos, query) {
            const suggestionBox = document.getElementById('suggestionBoxPublico');
            selectedIndexPublico = -1;
            ...
            const html = productos.map((p, index) => {
                const codigoHighlight = resaltarTextoPublico(p.codigo || '', queryLower);
                const descripcionHighlight = resaltarTextoPublico(p.descripcion || '', queryLower);
                
                return `
                    <div class="suggestion-item-publico px-4 py-2.5 hover:bg-indigo-50 cursor-pointer border-b border-gray-100 last:border-0 transition-colors"
                         data-index="${index}"
                         onclick="seleccionarProductoPublico('${p.codigo}')">
                        <div class="flex items-start gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="font-mono text-xs font-semibold text-indigo-600 mb-0.5">${codigoHighlight}</div>
                                <div class="text-sm text-gray-800 line-clamp-2">${descripcionHighlight}</div>
                                <div class="flex gap-3 mt-1 text-xs text-gray-500">
                                    ${p.ubicacion ? `<span>📍 ${p.ubicacion}</span>` : ''}
                                    ${p.um ? `<span>${p.um}</span>` : ''}
                                    ${p.fisico !== null ? `<span class="font-semibold ${p.fisico < 10 ? 'text-red-600' : 'text-gray-600'}">Stock: ${parseFloat(p.fisico).toFixed(2)}</span>` : ''}
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        function resaltarTextoPublico(texto, query) {
            if (!texto || !query) return texto;
            const regex = new RegExp(`(${query})`, 'gi');
            return texto.replace(regex, '<mark class="bg-yellow-200 font-semibold">$1</mark>');
        }

        function seleccionarProductoPublico(codigo) {
            const input = document.getElementById('searchInputPublico');
            input.value = codigo;
            const suggestionBox = document.getElementById('suggestionBoxPublico');
            suggestionBox.classList.add('hidden');
            suggestionBox.innerHTML = '';
        ...
        function navegarSugerenciasPublico(e) {
            const suggestionBox = document.getElementById('suggestionBoxPublico');
            const items = suggestionBox.querySelectorAll('.suggestion-item-publico');
            ...
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                selectedIndexPublico = (selectedIndexPublico + 1) % items.length;
                actualizarSeleccionPublico(items);
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                selectedIndexPublico = selectedIndexPublico <= 0 ? items.length - 1 : selectedIndexPublico - 1;
                actualizarSeleccionPublico(items);
            } else if (e.key === 'Enter') {
                e.preventDefault();
                if (selectedIndexPublico >= 0 && selectedIndexPublico < items.length) {
                    items[selectedIndexPublico].click();
                }
            }
        }

        function actualizarSeleccionPublico(items) {
            items.forEach((item, index) => {
                if (index === selectedIndexPublico) {
